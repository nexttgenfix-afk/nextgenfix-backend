...
exports.getMenuItems = async (req, res) => {
  try {
    ...
    if (filter === 'popular') {
      ...
      // Populate restaurant/chef details (only populate chefId when present in schema)
      const populateArray = [ { path: 'restaurantId', select: 'name location' } ];
      if (MenuItem.schema.path('chefId')) populateArray.push({ path: 'chefId', select: 'name kitchenName location' });
      menuItems = await MenuItem.populate(popularDishes, populateArray);
    } else {
      // For all other filters, use regular find
      let menuQuery = MenuItem.find(filterQuery).sort(sortOptions).populate('restaurantId', 'name location');
      if (MenuItem.schema.path('chefId')) menuQuery = menuQuery.populate('chefId', 'name kitchenName location');
      menuItems = await menuQuery.limit(parseInt(limit));
    }
